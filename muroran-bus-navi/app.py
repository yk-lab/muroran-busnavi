...
BaseTemplate.settings.update(
    {"filters": {"tojson": lambda content: json.dumps(content)}}
)
...
def index():
    ...
    return template(
        ...
        page={
            ...
            "discription": "道南バスの室蘭市内線のバス停地図の表示やバスの時刻表検索を"
            "行えます。平日・土日祝日にダイヤ対応。",
        },
        params={
            "day": jst_now.date(),
            "time": "%02d:%02d" % (jst_now.hour, jst_now.minute // 10 * 10),
        },
        autoescape=True,
    )
...
def company_detail(id, db):
    ...
    if company:
        ...
        return template(
            "company/details.tpl.html", company=company, autoescape=True
        )
...
def stop_search(db):
    ...
    if query is not None and query != "":
        ...
        stopnames = (
            db.query(StopName)
            .filter(StopName.name.contains(query))
        ...
        if format == "json":
            ...
            return json.dumps(
                [
                    {
                        "stopname_id": stopname.id,
                        "stop": stopname.stop.to_dict(),
                    }
                    for stopname in stopnames
                ]
            )
        return template(
            "stop/search_list.tpl.html",
            query=query,
            stopnames=stopnames,
            autoescape=True,
        )
    return template(
        "stop/search.tpl.html",
        page={"title": "バス停検索 - むろらんバスなび"},
        autoescape=True,
    )
    ...
    if request.params.f_id and request.params.t_id:
        ...
        if request.params.day:
            ...
            if (
                ...
                dt = datetime.combine(
                    date(int(d[0]), int(d[1]), int(d[2])), time_obj
                )
        # monday, tuesday, wednesday, thursday, friday, saturday, sunday
        a_service_dates = db.query(ServiceDate).filter(
            ServiceDate.date == dt.astimezone(JST).date()
        )
        if dt.weekday() == 0:
            a_service_weekday = db.query(Service).filter(
                Service.monday == 1,
                Service.start_date <= dt.astimezone(UTC),
                Service.end_date >= dt.astimezone(UTC),
            )
        elif dt.weekday() == 1:
            a_service_weekday = db.query(Service).filter(
                Service.tuesday == 1,
        ...
        trips = db.query(Trip.id).filter(
            Trip.service_code.in_([k for k, s in a_service_code.items() if s])
        )
        f_stop_positions = db.query(StopPosition.id).filter(
            StopPosition.stop_code == request.params.f_id
        )
        t_stop_positions = db.query(StopPosition.id).filter(
            StopPosition.stop_code == request.params.t_id
        )
        f_stop_times = db.query(StopTime.trip_code).filter(
            StopTime.stop_code.in_(f_stop_positions),
            StopTime.trip_code.in_(trips),
            StopTime.departure_time > time_sec,
        )
        t_stop_times = (
            db.query(StopTime)
            .filter(
                StopTime.stop_code.in_(t_stop_positions),
                StopTime.trip_code.in_(trips),
                StopTime.trip_code.in_(f_stop_times),
            )
        ...
        for t_stop_time in t_stop_times:
            st = (
                ...
                .filter(
                    StopTime.stop_code.in_(f_stop_positions),
                    StopTime.trip_code == t_stop_time.trip_code,
                    StopTime.stop_sequence < t_stop_time.stop_sequence,
                    StopTime.departure_time > time_sec,
                )
        ...
        stopnames = (
            db.query(StopName)
            .filter(StopName.name.contains(request.params.to_q))
            .order_by(func.char_length(StopName.name))
        )
        return template(
            "stop_times/select_stop.tpl.html",
            params=request.params,
            select="t_id",
            stopnames=stopnames,
            request=request,
        ...
        stopnames = (
            db.query(StopName)
            .filter(StopName.name.contains(request.params.from_q))
            .order_by(func.char_length(StopName.name))
        )
        return template(
            "stop_times/select_stop.tpl.html",
...
def passing_times(trip_id, db):
    ...
    try:
        ...
        pass
    dt = dt if dt is not None else pendulum.now()

    origin = (
        int(request.params.origin)
        if request.params.origin and request.params.origin.isdigit()
        else 0
    )
    if request.params.destination and request.params.destination.isdigit():
        ...
        stop_times = db.query(StopTime).filter(
            StopTime.trip_code == trip_id, StopTime.stop_sequence >= origin
        )
...
@app.post("/feedback/")
def post_feedback():
    if not request.get_header("Referer").startswith(
        "https://muroran.bus-navi.yk-lab.net/"
    ):
        # 投稿元が違うため
        return HTTPError(404, "page not found.")
    if (
        not request.get_header("User-Agent")
        or len(request.get_header("User-Agent")) < 10
    ):
        # user agent チェック
        # ここは時間を見てしっかりやりたい
        return HTTPError(404, "page not found.")
    ...
    client = pymongo.MongoClient(
        host=app.config["FEEDBACK.DB.HOST"],
        port=app.config["FEEDBACK.DB.PORT"],
    )
        ...
        fb.count_documents(
            {
                ...
                "body.registered_on": {
                    "$gt": datetime.now(UTC) - timedelta(hours=3)
                },
            }
        )
        > 5
    ...
    if (
        fb.count_documents(
            {
                "body.registered_on": {
                    "$gt": datetime.now(UTC) - timedelta(seconds=5)
                }
            }
        )
        > 1
    ...
    if app.config.get("FEEDBACK.SLACK.TOKEN"):
        ...
        slack.chat.post_message(
            app.config["FEEDBACK.SLACK.CHANNEL"],
            "Feedback: " + str(result.inserted_id),
        )
        ...
        slack.chat.post_message(
            app.config["FEEDBACK.SLACK.CHANNEL"], "File: " + file_url
        )
...
def api_1_stop(id, db):
    ...
    class NeighborStopSchema(Schema):
        dist = fields.Integer()
        stop = fields.Nested(StopSchema)

    return json.dumps(
        {
            "data": StopSchema().dump(stop).data,
            "neighbor_stops": NeighborStopSchema(many=True).dump(n_stops).data,
        }
    )
...
def api_v1_0_stop_search(db):
    ...
    if search_type == "latlng":
        try:
            ...
            response.status = 400
            return json.dumps(
                {"Error": [{"message": "Missed Required Params"}]}
            )
        try:
        ...
        if lat >= -90 and lat <= 90 and lng >= -180 and lng <= 180:
            ...
            quadkeys = QuadkeyUtils.neighbors_quadkey(
                mercantile.quadkey_to_tile(
                    QuadkeyUtils.cut_key(
                        mercantile.quadkey(*mercantile.tile(lng, lat, dl)), dl
                    )
                )
            )
            ...
            for ns in near_stops.all():
                ...
                if dist <= radius:
                    stops.append(
                        {"stop_data": ns, "dist": int(round(dist, -1))}
                    )
            if stops:
        ...
        response.status = 400
        return json.dumps({"Error": [{"message": "Unsupported Type"}]})
    response.status = 400
    return json.dumps({"Error": [{"message": "Bad Request"}]})


@app.route("/<test:re:.*>")
def static_index(test):
    return template("../client/dist/index.html")
...
if __name__ == "__main__":
    ...
    app.run(
        ...
        reloader=app.config.get("RELOAD", False),
    )
    # else:
